...
public class Number4choosenum1 extends View {
    Path path,anopath;
    Paint paint;
...
//    Path shadowpath;
    ...
    public static Number4choosenum1 instance;
    public static  Number4choosenum1 getInstance(Context context, Display display){
        if (instance == null) {
            instance=new Number4choosenum1(context, display);
        }
        return instance;
    }
    public Number4choosenum1(Context context) {
        super(context);
    }
    public Number4choosenum1(Context context,Display display) {
        super(context);
        this.init(display);
    }
    public void init(Display display) {
        ...
        rightcheckanimator = ObjectAnimator.ofFloat(Number4choosenum1.this, "rightcheckprogress", 0.0f, 1.0f);
        rightcheckanimator.setDuration(750);
        checkanimator = ObjectAnimator.ofFloat(Number4choosenum1.this, "checkprogress", 0.0f, 1.0f);
        checkanimator.setDuration(750);
        shadowanimator = ObjectAnimator.ofFloat(Number4choosenum1.this, "shadowprogress", 0.0f, 1.0f);
...
//        this.setBackgroundColor(Color.WHITE);
        ...
        for(int i=0;i<3;i++){
            ...
            regions[i].typename="light_green";
            regions[i].id=6+i;
        }
        for(int i=0;i<3;i++){
            rects[i+3]=new RectF((47+10*i)*onep_screen_w,2*onep_screen_h,
                    (57+10*i)*onep_screen_w,2*onep_screen_h+10*onep_screen_w);
        }
        for(int i=0;i<3;i++){
            path = new Path();
            path.addRect(rects[i+3], Path.Direction.CW);
            regions[i+3] = new Touchregion(path);
            regions[i+3].point_x=(int)((52+10*i)*onep_screen_w);
            regions[i+3].point_y=(int)(2*onep_screen_h+5*onep_screen_w);
            regions[i+3].typename="yellow";
            regions[i+3].id=5+i;
        }
        for(int i=0;i<4;i++){
            rects[i+6]=new RectF((2+10*i)*onep_screen_w,9*onep_screen_h,
                    (12+10*i)*onep_screen_w,9*onep_screen_h+10*onep_screen_w);
        }
        for(int i=0;i<4;i++){
            path = new Path();
            path.addRect(rects[i+6], Path.Direction.CW);
            regions[i+6] = new Touchregion(path);
            regions[i+6].point_x=(int)((7+10*i)*onep_screen_w);
            regions[i+6].point_y=(int)(9*onep_screen_h+5*onep_screen_w);
            regions[i+6].typename="red";
            regions[i+6].id=3+i;
        }
        for(int i=0;i<4;i++){
            rects[i+10]=new RectF((47+10*i)*onep_screen_w,9*onep_screen_h,
                    (57+10*i)*onep_screen_w,9*onep_screen_h+10*onep_screen_w);
        }
        for(int i=0;i<4;i++){
            path = new Path();
            path.addRect(rects[i+10], Path.Direction.CW);
            regions[i+10] = new Touchregion(path);
            regions[i+10].point_x=(int)((52+10*i)*onep_screen_w);
            regions[i+10].point_y=(int)(9*onep_screen_h+5*onep_screen_w);
            regions[i+10].typename="blue";
            regions[i+10].id=6+i;
        }
        for(int i=0;i<14;i++){
            regions[i].isright=true;
        }
        regions[0].isright=regions[2].isright=regions[3].isright=regions[5].isright=regions[7].isright=regions[8].isright=
                regions[10].isright=regions[12].isright=regions[13].isright=false;

        //6个数字选择区域
        chooserectfs= new RectF[6];
        for(int i=0;i<6;i++){
            chooserectfs[i]=new RectF((15*i+4)*onep_screen_w,25*onep_screen_h-5*onep_screen_w,
                    (15*i+14)*onep_screen_w,25*onep_screen_h+5*onep_screen_w);
        ...
        for(int i=0;i<6;i++){
            ...
            chooseregions[i].point_x=(int)((15*i+9)*onep_screen_w);
            chooseregions[i].point_y=(int)(25*onep_screen_h);
            chooseregions[i].id=4+i;
        }
...
//       shadowpaint.setPathEffect(createPathEffect(shadowlength, shadowprogress, 0.0f));
    ...
    public void onDraw(Canvas c) {
        ...
        if(isshadow){
            ...
            shadowrect = new Rect(0, (int) (25*onep_screen_h+8*onep_screen_w-shadowprogress * 16 * onep_screen_w),
                    (int)(1.2*Screen_w), (int)(25*onep_screen_h+8*onep_screen_w));
