...
public class Number4choosenum2 extends View {
    Path path,anopath;
    Paint paint;
...
//    Path shadowpath;
    ...
    public static Number4choosenum2 instance;
    public static  Number4choosenum2 getInstance(Context context, Display display){
        if (instance == null) {
            instance=new Number4choosenum2(context, display);
        }
        return instance;
    }
    public Number4choosenum2(Context context) {
        super(context);
    }
    public Number4choosenum2(Context context,Display display) {
        super(context);
        this.init(display);
    }
    public void init(Display display) {
        ...
        rightcheckanimator = ObjectAnimator.ofFloat(Number4choosenum2.this, "rightcheckprogress", 0.0f, 1.0f);
        rightcheckanimator.setDuration(750);
        checkanimator = ObjectAnimator.ofFloat(Number4choosenum2.this, "checkprogress", 0.0f, 1.0f);
        checkanimator.setDuration(750);
        shadowanimator = ObjectAnimator.ofFloat(Number4choosenum2.this, "shadowprogress", 0.0f, 1.0f);
...
//        this.setBackgroundColor(Color.WHITE);
        ...
        for(int i=0;i<4;i++){
            ...
            regions[i+4].typename="light_green";
            regions[i+4].id=9-i;
        }
        for(int i=0;i<4;i++){
            rects[i+8]=new RectF((2+10*i)*onep_screen_w,9*onep_screen_h,
        ...
        for(int i=0;i<4;i++){
            ...
            regions[i+8].point_x=(int)((7+10*i)*onep_screen_w);
            regions[i+8].point_y=(int)(9*onep_screen_h+5*onep_screen_w);
            regions[i+8].typename="orange";
            regions[i+8].id=2*(i+1);
        }
        for(int i=0;i<4;i++){
            rects[i+12]=new RectF((47+10*i)*onep_screen_w,9*onep_screen_h,
                    (57+10*i)*onep_screen_w,9*onep_screen_h+10*onep_screen_w);
        }
        for(int i=0;i<4;i++){
            path = new Path();
            path.addRect(rects[i+12], Path.Direction.CW);
            regions[i+12] = new Touchregion(path);
            regions[i+12].point_x=(int)((52+10*i)*onep_screen_w);
            regions[i+12].point_y=(int)(9*onep_screen_h+5*onep_screen_w);
            regions[i+12].typename="green";
            regions[i+12].id=1+2*i;
        }
        for(int i=0;i<16;i++){
            regions[i].isright=true;
        }
        regions[3].isright=regions[6].isright=regions[11].isright=regions[14].isright=false;

        //四个数字选择区域
        chooserectfs= new RectF[4];
        for(int i=0;i<4;i++){
            chooserectfs[i]=new RectF((20*i+10)*onep_screen_w,25*onep_screen_h-5*onep_screen_w,
                    (20*i+20)*onep_screen_w,25*onep_screen_h+5*onep_screen_w);
